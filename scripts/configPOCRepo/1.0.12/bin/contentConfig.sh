...
packetDecoderContent () {
    ...
    for i in $(find /tmp/configPOC -type f -iname "*.nwr"); do 
        ...
        num=$(echo $i | grep -c REQUIRES_CUSTOMIZATION)
        if [[ $num -gt 0 ]]; then
            echo -e "\nIR nwr file that requires customization is present. Please edit, rename file, and run the script again. Other rules will be processed."
        ...
        for line in $(grep "type=application" $i); do
            ...
            err=""
            # Process rule
            err=$(curl -s -u admin:netwitness http://192.168.1.113:50104/decoder/config/rules/application/?force-content-type=text/plain -d msg=merge --data-urlencode name=$line | grep -o 400)
