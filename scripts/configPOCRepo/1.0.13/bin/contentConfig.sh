...
concentratorContent () {
    ...
    if [ -e $thumb/index-concentrator-custom.xml ]; then
        ...
        cp -u $thumb/index-concentrator-custom.xml /etc/netwitness/ng/backups/      
    else
...
packetDecoderContent () {
    ...
    # Tell them what we are doing
    echo "Backing up and pushing decoder content..."
    sleep 2
    ...
    # Modify line seperator
    OFS=$IFS
    IFS=$'\r\n'

    # Process nwr files with multiple lines
    # Check for IR custom file
    if [[ $(find /tmp/configPOC -type f -iname "*REQUIRES_CUSTOMIZATION*" | wc -l) -gt 0 ]]; then
        echo -e "\nIR nwr file that requires customization is present. Please edit, rename file, re-zip, and run the script again."
        echo "Please hit Enter to continue..."
        read
        rm -rf /tmp/configPOC
        exit 1
    fi 
    for i in $(find /tmp/configPOC -type f -iname "*.nwr"); do 
        for line in $(grep "type=application" $i); do
            ((cnt++))
            # Clear errors
            err=""
            # Process rule
            err=$(curl -s -u admin:netwitness http://localhost:50104/decoder/config/rules/application/?force-content-type=text/plain -d msg=merge --data-urlencode name=$line | grep -o 400)
            # Check for error and the write broken rule to file
            if [[ $err == "400" ]]; then
                echo $line >> nwrParseErrors.log
            fi
        done
    done
    # Restore the feild seperator
    IFS=$OFS
    ...
    restart nwdecoder
    # Give IR notice to user
    echo "If you just pushed IR content, you will need to enable the parsers in the SA interface."
    echo "Hit enter to continue..."
    read
} 
